...
export function SectorPie({ holdings }) {
  ...
  const data = Object.entries(sectors).map(([name, value], i) => ({
    name,
    value,
  ...
  return (
    <motion.div 
      ...
      className="h-full"
    >
      <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted mb-6">Sector Diversification</h3>
      ...
      <div className="relative mb-6 flex justify-center">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <span className="text-[10px] font-black uppercase tracking-wider text-muted opacity-60">Sectors</span>
          <span className="text-sm font-bold text-white">{data.length}</span>
        </div>
      </div>

      {/* Legend list */}
      <div className="space-y-3.5 px-1 max-h-[250px] overflow-y-auto no-scrollbar">
        {data.map((d) => (
          <div key={d.name} className="flex items-center justify-between group cursor-default">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-white font-bold text-[11px] group-hover:text-accent transition-colors truncate max-w-[120px]">{d.name}</span>
            </div>
